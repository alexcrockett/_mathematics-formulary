...
Square root functions are just a variety of power functions. This is evident both in the behavior but, it is also evident in the notation.

Starting with the notation, $\sqrt{x}$ is also written $x^{\frac{1}{2}}$, we will see that rewriting radicals in this way will help mitigate a number of problems when working with these functions.
